...
int activateWindow(Window window);
...
int activateWindow(/* Display display,*/ Window window) {
  Display *display  = XOpenDisplay(NULL);
  int ret;
  if (display == NULL) {
    fprintf(stderr, "Errore nell'apertura del Display !!!\n");    
  ...
  if (ret == BadValue) {
    printf("%s %d: %s\n", __FILE__, __LINE__, "BadValue");
  } else if (ret == BadWindow) {
    printf("%s %d: %s\n", __FILE__, __LINE__, "BadWindow");
  } else if (ret == BadRequest) {
    printf("%s %d: %s\n", __FILE__, __LINE__, "BadRequest");
  }
  XFlush(display);
  XCloseDisplay(display);
  return 0;
